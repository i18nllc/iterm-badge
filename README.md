...
| **`iterm-badge-auto`** | **Automatic status lamp.** Hooks flip the badge to <br>• `🔄 working` when you submit a prompt, <br>• `✅ done` when Claude finishes, <br>• `⏳ needs you` when Claude waits on your input — and clear it when the session ends. | Nothing. Install it and it's on. |
